...
TOTAL_CAPITAL      = _env_float("TOTAL_CAPITAL", 500000)
RISK_PER_TRADE_PCT = _env_float("RISK_PER_TRADE_PCT", 1.0)
LOT_SIZE           = _env_int("LOT_SIZE", 50)
MAX_LOTS           = _env_int("MAX_LOTS", 5)

# ─── HARD FILTERS — NO TRADE CONDITIONS ────────────────
FILTER_IV_RANK_MAX   = 80
FILTER_ADX_MIN       = 15
FILTER_EXPIRY_CUTOFF = "14:00"

# ─── OI CONFLUENCE WEIGHTS ─────────────────────────────
OI_CONFLUENCE_MAX_SCORE = 3
...
# --- EXECUTION --------------------------------------------------------------
# Paper mode has been removed. The bot always places real broker orders.
# Ensure KITE_API_KEY / KITE_API_SECRET / access_token.txt are valid before starting.
LIVE_MODE             = True
ENTRY_ORDER_TYPE      = "MARKET"   # MARKET or LIMIT
EXIT_ORDER_TYPE       = "MARKET"   # MARKET or LIMIT
ORDER_PRODUCT         = "MIS"
ENABLE_SL_ORDER       = _env_bool("ENABLE_SL_ORDER", True)
...
# --- PERSISTENCE / INFRA ----------------------------------------------------
STATE_DB_PATH          = "state/bot_state.sqlite"
LOG_DIR                = "logs"
# BUG FIX: was "oi_baseline.json" (root dir), now consistent with state/ folder
OI_BASELINE_PATH       = "state/oi_baseline.json"
...
DAILY_LOSS_LIMIT = -5000   # ₹5,000 max daily loss; set 0 to disable
